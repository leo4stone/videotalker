...
class MarkerPlayer {
  ...
  createPlayerMarker(annotation) {
    ...
    editButtonsContainer.className = 'player-marker-edit-buttons';

    // 创建编辑打点按钮
    const editAnnotationButton = document.createElement('div');
    editAnnotationButton.className = 'player-annotation-edit-btn';
    editAnnotationButton.title = '编辑打点';
    editAnnotationButton.dataset.annotationId = annotation.id;
    editAnnotationButton.innerHTML = `
    ...
    // 创建编辑标记按钮
    const editMarkerButton = document.createElement('div');
    editMarkerButton.className = 'player-marker-edit-btn';
    editMarkerButton.title = '编辑标记';
    editMarkerButton.dataset.annotationId = annotation.id;
    editMarkerButton.innerHTML = `
      <svg width="10" height="10" viewBox="0 0 16 16" fill="currentColor">
    ...
    editAnnotationButton.addEventListener('click', (e) => {
      e.stopPropagation();
    ...
    // 阻止编辑打点按钮的其他鼠标事件
    editAnnotationButton.addEventListener('mousedown', (e) => e.stopPropagation());
    editAnnotationButton.addEventListener('mouseup', (e) => e.stopPropagation());

    // 复用annotation-popup的编辑标记按钮事件逻辑
    editMarkerButton.addEventListener('click', (e) => {
      e.stopPropagation();
      // 如果打点没有marker数据，先初始化
  ...
  updateExistingMarker(markerElement, annotation) {
    ...
    // 更新编辑按钮的annotation数据
    const editAnnotationButton = markerElement.querySelector('.player-annotation-edit-btn');
    const editMarkerButton = markerElement.querySelector('.player-marker-edit-btn');

    if (editAnnotationButton) {
      editAnnotationButton.dataset.annotationId = annotation.id;
      // 重新绑定编辑打点按钮事件
      const newEditAnnotationButton = editAnnotationButton.cloneNode(true);
      editAnnotationButton.parentNode.replaceChild(newEditAnnotationButton, editAnnotationButton);

      newEditAnnotationButton.addEventListener('click', (e) => {
        e.stopPropagation();
        if (window.annotationManager) {
          window.annotationManager.showAnnotationDetailModal(annotation.id);
        }
      ...
      newEditAnnotationButton.addEventListener('mousedown', (e) => e.stopPropagation());
      newEditAnnotationButton.addEventListener('mouseup', (e) => e.stopPropagation());
    }

    if (editMarkerButton) {
      editMarkerButton.dataset.annotationId = annotation.id;
      // 重新绑定编辑标记按钮事件
      const newEditMarkerButton = editMarkerButton.cloneNode(true);
      editMarkerButton.parentNode.replaceChild(newEditMarkerButton, editMarkerButton);

      newEditMarkerButton.addEventListener('click', (e) => {
        e.stopPropagation();
        // 如果打点没有marker数据，先初始化
        if (!annotation.marker) {
          annotation.marker = {
